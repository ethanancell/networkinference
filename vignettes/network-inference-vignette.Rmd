---
title: "Vignette for post-selection inference with a single realization of a network"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{network-inference-vignette}
...
Now, let's use the `split_matrix()` function from the package to split this adjacency matrix $A$ into our train and test adjacency matrices. As arguments, we must provide our distributional setting (Gaussian), our splitting parameter $\epsilon = 0.5$, as well as our known variance $\tau^2 = 4^2$.

```{r, fig.width=6, fig.height = 3}
gaussian_split_results <- split_matrix(A, distribution = "gaussian", 
                                       epsilon = 0.5, tau = tau)
...
Now, we can use the `split_matrix()` function from the package to split this into our train and test adjacency matrices. To split with a parameter $\gamma = 0.15$, we will pass in `gamma = 0.15` into the function, and specify our distributional setting.

```{r, fig.width=6, fig.height = 3}
bernoulli_split_results <- split_matrix(A, distribution = "bernoulli",
                                        gamma = 0.15)
